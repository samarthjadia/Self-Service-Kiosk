...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( recipe.getIngredients().size() == 0 ) {
            ...
            return new ResponseEntity( errorResponse( recipe.getName() + " does not contain any ingredients." ),
                    HttpStatus.CONFLICT );
        }
        ...
        for ( int i = 0; i < ingredients.size(); i++ ) {
            if ( ingredients.get( i ).getAmount() <= 0 ) {
                ...
                return new ResponseEntity( errorResponse( "Ingredient '" + ingredients.get( i ).getName()
                        + "' has invalid units. Units must be greater than 0" ), HttpStatus.CONFLICT );
            }
        }
